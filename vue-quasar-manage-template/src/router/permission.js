...
import {getItem,set, getToken, getUserRoleNames} from '@/utils/authUtil'
...
router.beforeEach((to, from, next) => {
  ...
  const userRole = getUserRoleNames()
  // 存在 token 说明已经登录
  if (token) {
    // 登录过就不能访问登录界面
    if (to.path === '/login') {
...
function handleTagViewAndBreadcrumbsAndKeepAlive (to) {
  if (to.name != null) {
    ...
    let tagViewOnSS = []
    JSON.parse(getItem('tagView')) === null ? set('tagView', '[]') : tagViewOnSS = JSON.parse(getItem('tagView'))
